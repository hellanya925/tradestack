...
<html>
    <head>
        ...
        import { collection, getDocs, getFirestore, onSnapshot, query } from "https://www.gstatic.com/firebasejs/9.7.0/firebase-firestore.js";
      
        const firebaseConfig = {
          apiKey: "AIzaSyC4kDCM9XPEqnVj5_iMVEAR7IeOQfTZ58g",
          authDomain: "khod-bafdc.firebaseapp.com",
          projectId: "khod-bafdc",
          storageBucket: "khod-bafdc.appspot.com",
          messagingSenderId: "460309137329",
          appId: "1:460309137329:web:2962a7bec4bf5a6bf49f90",
          measurementId: "G-RR61158V8P"
        };
        ...
        const q = query(collection(db, "latest_rates"));
        const contentContainer = document.getElementById('khod_latest_rates');
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const rates = [];
            querySnapshot.forEach((doc) => {
                rates.push(doc.data());
            });
            rates.sort((leftRate, rightRate) => rightRate.rate - leftRate.rate)

            contentContainer.innerHTML = ""
            let col;
            rates.forEach((rate) => {
                let colorClass = ""
                if(rate.rateChange > 0)
                    colorClass = "increasingRate"
                if(rate.rateChange < 0)
                    colorClass = "decreasingRate"
                if(rate.shortDiscription == 'LiraRate'){
                    col = `<div class='col-sm-12 col-xs-12 col-lg-3'>
                    <p class="rate ${colorClass}">Black Market</p>
                    <input type="text" class='form-control bm' value="${rate.rate+' '+rate.targetCurrency}" disabled>
                    <p class="lastUpdate">${timeSince(new Date(parseInt(rate.timestamp)))} ago</p>
                    </div>
                `;
                }else if(rate.shortDiscription == 'Sayrafa'){
                    col = `<div class='col-sm-12 col-xs-12 col-lg-3'>
                    <p class="rate ${colorClass}">Sayrafa</p>
                    <input type="text" class='form-control sayrafa' value="${rate.rate+' '+rate.targetCurrency}" disabled>
                    <p class="lastUpdate">${timeSince(new Date(parseInt(rate.timestamp)))} ago</p>
                    </div>
                `;
                }else if(rate.shortDiscription == 'OMT'){
                    col = `<div class='col-sm-12 col-xs-12 col-lg-3'>
                    <p class="rate ${colorClass}">OMT</p>
                    <input type="text" class='form-control omt' value="${rate.rate+' '+rate.targetCurrency}" disabled>
                    <p class="lastUpdate">${timeSince(new Date(parseInt(rate.timestamp)))} ago</p>
                    </div>`;
                }else{
                    col = ''
                }
                contentContainer.innerHTML += col;
            })
        });
      </script>
      <style>
