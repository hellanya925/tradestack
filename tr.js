...
function setProperty(property,value){
    ...
    load.style[`${property}`] = value;
}
function addPropertyDivs(){
    let keys = Object.keys(properties),
    values = Object.values(properties);
    let node,
    propsC = document.getElementById('propC');
    for(var i = 0;i < keys.length;i++){
        node = document.createElement('div');
        node.classList.add('propC');
        if(values[i] == ''){
            values[i] = 'text'
        }
        node.innerHTML = '<div class="col" id="'+keys[i]+'">'+keys[i]+'</div><div class="col">\
        <input type="'+values[i]+'" class="form-control" property="'+keys[i]+'" onchange=setProperty(this.getAttribute("property"),this.value)></div>';
        propsC.append(node);
    }
}